 ...
 * @param offset
 */
function myPulish(path,currentPage) {
	// 清空tbody中数据
	$("#publishTable").html('');
	// 清空分页数据
	$("#publishPage").html('');
	$.ajax({
		...
		success: function(data){
			...
			/*var personString='{"name":"zhangsan","sex":"男","age":22}';
			...
			$.each(data.notices, function(k ,v){
				...
				if (k % 2 == 0)
					$tr.addClass('success');
				var index = Number((page.currentPage-1) * page.pageNum)+ Number(1) + Number(k);
			...
			var spanStr = '<span>共' + page.totalRecord + '条记录 共' + page.totalPage + '页 当前第' + page.currentPage + '页</span>';
			var ulStr = '<ul class="pagination" style="float: right;margin: 0;">';
				...
				if (page.currentPage != 1) {
					ulStr += '<li><a href="javascript:void(0)" onclick="myPulish(\'' + path + '\', '+(Number(page.currentPage)+Number(1))+')">上一页</a></li>';
				} else {
					ulStr += '<li class="disabled"><a href="javascript:void(0)">上一页</a></li>';
				}
				if (page.currentPage != page.totalPage) {
					ulStr += '<li><a href="javascript:void(0)" onclick="myPulish(\'' + path + '\', '+(Number(page.currentPage)+Number(1))+')">下一页</a></li>';
				} else {
					ulStr += '<li class="disabled"><a href="javascript:void(0)">下一页</a></li>';
				}
				ulStr += '<li><a href="javascript:void(0)" onclick="myPulish(\'' + path + '\', '+Number(page.totalPage)+')">尾页</a></li>';
 ...
 * @param offset
 */
function myReceive(path, currentPage) {
	// 清空tbody中数据
	$("#receiveTable").html('');
	// 清空分页数据
	$("#receivePage").html('');
	$.ajax({
		...
		success: function(data){
			...
			$.each(data.notices, function(k ,v){
				...
				var $titleTd = $('<td><a href = "' + path + '/notice/notice/detail.action?id=' + v.id + '&type=0">' + v.title + '</a></td>');
				var $contenTd = $('<td>' + v.content + '</td>');
				var $realNameTd = $('<td>' + v.realName + '</td>');
